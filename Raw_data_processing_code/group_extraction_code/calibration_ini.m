function [Output, Calib, calib_status] = calibration_ini(parameter, sensor_name, sensor_thickness)
    ...
    %
    disp('Running calibration_ini.m')
    ...
    if strcmp(calib_status, 'Yes')
        ...
        if strcmp(calib_status, 'No')
            ...
            I_crossed_nobias_field = Func_load_plot_calibration_data('Select the 0V Crossed data', '0V Crossed', 'southeast');
            ...
            fig_selected_Efield = figure('Name', 'Selected E-field');
            movegui(fig_selected_Efield, "south");
            ...
            Edge_left_sel = Edge_left(points(1):points(2)); % Crop edge arrays
            ...
            xs2 = (0:(Edge_right_median - Edge_left_median)) / T_median * sensor_thickness * 1E-3; %[m]
            Output.integral_Efield = trapz(xs2, y);
            int_E_all = num2str(round(Output.integral_Efield));
...
function [I_calibration_field] = Func_load_plot_calibration_data(prompt_message, figure_name, figure_pos)
    ...
    %
    % This function prompts the user to select a .txt file for either 0V parallel
    % or crossed polarizer calibration data, reads the data, reshapes it into a 2D
    % matrix, and then plots it in a specified figure window.
    %
    % Inputs:
    %   prompt_message: A string containing the message displayed in the prompt
    %                   asking the user to select a file.
    %   figure_name: A string specifying the name of the figure window where the
    %                data will be plotted.
    %   figure_pos: A string indicating the position where the figure window
    %               should be moved on the screen (e.g., 'center', 'northeast').
    %
    % Outputs:
    %   I_calibration_field: A 2D matrix containing the reshaped calibration data.
