...
for i=1:N
    ...
    title(['Pockels average profile - sensor=' Output.sensor_name '@' bias_string 'V_' flux_string 'mA'])
    xlabel('thickness (mm)')
    ylabel('E field (V/m)')
...
%                 diff=str2num(Output.bias)-Output.integral_Efield;
    ...
    figure(3)
    plot(Output.bias,Output.integral_Efield,'o');
    ...
    figure(3)
    plot(Output.bias,Output.integral_Efield,'s','displayname',[bias_string 'V_' flux_string 'mA']);
    title(['Integral E-field - sensor=' Output.sensor_name])
    xlabel('Bias (V)')
    ylabel('Integral E-field (V)')   
    box       
